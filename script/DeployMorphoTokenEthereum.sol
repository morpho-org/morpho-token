...
contract DeployMorphoTokenEthereum is Script {
    ...
    bytes32 public constant WRAPPER_SALT = 0;
    ...
    function run() public returns (address, address) {
        ...
        address tokenImplementation = address(new MorphoTokenEthereum{salt: IMPLEMENTATION_SALT}());

        console.log("Deployed token implementation at", tokenImplementation);

        // Deploy Token proxy
        MorphoTokenEthereum token =
            MorphoTokenEthereum(address(new ERC1967Proxy{salt: PROXY_SALT}(deploymentImplementation, hex"")));

        console.log("Deployed token proxy at", address(token));

        // Deploy Wrapper
        address wrapper = address(new Wrapper{salt: WRAPPER_SALT}(address(token)));
